...
See the [current version-by-feature comparison](doc/dxf-conformance/version-feature-matrix.md) and its [machine-readable coverage ledger](doc/dxf-conformance/coverage.json). Each entry distinguishes tested field-level support, partial typed models, known losses, rejected writer profiles and opaque preservation. The comparison is generated and checked in CI; its production commit and evidence are pinned. The [merged 14 September checkpoint](doc/dxf-conformance/checkpoint-merged-2026-09-14.md) records the 189-row/nine-profile comparison and verified 16,765-case production snapshot after PR #75.
