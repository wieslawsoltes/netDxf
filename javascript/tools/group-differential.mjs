...
const output=path.join(javascriptRoot,'artifacts/group-differential',configuration);fs.mkdirSync(output,{recursive:true});
const oracle=new OracleClient({args:[path.join(oracleRoot,'GeometryOracle.dll')]});
try {
  if(!corpus.length||new Set(corpus.map(p=>p.name)).size!==corpus.length)throw new Error('Empty or duplicated group comparison identities.');
  ...
  for(const probe of corpus) {
    ...
    if(differences.length){report.stats.failures+=differences.length;category.failures+=differences.length;report.failures.push({name:probe.name,category:probe.category,differences});
      if(report.failures.length<=12)console.error('Group mismatch',JSON.stringify(report.failures.at(-1)));}
  }
