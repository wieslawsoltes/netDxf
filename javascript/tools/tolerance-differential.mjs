...
try{
  const corpus=toleranceCorpus();
  if(corpus.length!==634||new Set(corpus.map(p=>p.name)).size!==634||corpus.reduce((n,p)=>n+p.request.steps.length,0)!==4998)
    throw new Error('Incomplete or duplicate tolerance corpus.');
  ...
  if(runtimeFingerprint()!==proof.runtimeFingerprint||verificationFingerprint()!==proof.verificationFingerprint){report.completed=false;report.fatal='Executable source changed during tolerance qualification.';}
  const directory=path.join(javascriptRoot,'artifacts/tolerance-differential',configuration);fs.mkdirSync(directory,{recursive:true});
