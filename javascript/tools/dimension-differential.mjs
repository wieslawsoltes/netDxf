...
try {
  const corpus = dimensionCorpus();
  if (corpus.length !== 3182 || new Set(corpus.map(p=>p.name)).size !== corpus.length ||
      corpus.reduce((n,p)=>n+p.request.steps.length,0) !== 14341)
    throw new Error('Incomplete or duplicate dimension corpus.');
  ...
  for (const probe of corpus) {
    ...
    if (!Array.isArray(expected) || expected.length !== probe.request.steps.length ||
      ...
      throw new Error('Incomplete dimension result: '+probe.name);
  ...
  if (runtimeFingerprint() !== proof.runtimeFingerprint || verificationFingerprint() !== proof.verificationFingerprint) {
    report.completed=false; report.fatal='Executable source changed during dimension qualification.';
  }
  const directory=path.join(javascriptRoot,'artifacts/dimension-differential',configuration);
