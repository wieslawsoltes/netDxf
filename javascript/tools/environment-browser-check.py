# Focused environment geometry browser qualification. The complete browser gates
...
report={'scope':'environment-io-only','fullSuite':False,'completed':False,'executionMode':'inline-native-esm','digestProvider':'host-sha256','inputTransport':'chunked-json','inputChunkCharacters':262144}
try:
    proof=fingerprints()
    corpus=json.loads((ROOT/('artifacts/environment-browser/'+configuration+'/corpus.json')).read_text(encoding='utf-8'))
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if report.get('comparisons') != 1760:
                report['completed']=False
                report['fatal']='Incomplete environment browser comparison count.'
    ...
    output=ROOT/'artifacts/environment-browser'/configuration/'results.json'
