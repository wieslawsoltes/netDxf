  ...
  return input.steps.map(step=>{
    ...
    try{
      switch(step.method){
        ...
        case 'read':entity=codec['Read'+input.kind](reader,document,input.entityCode);break;
        case 'write':codec['Write'+input.kind](writer,input.version,entity);break;
        ...
        case 'validate':codec[input.kind==='Light'?'ValidateLightVersions':input.kind==='LwPolyline'?'ValidateLwPolylineFidelity':'ValidateAcisEntities'](document);break;
    ...
    return {ok:true,value:{error,reader:{code:reader.Code,value:value(reader.Value,lastType),position:reader.CurrentPosition},entity:wire(entity),output:bytesToBase64(binary?outputBytes.ToArray():new TextEncoder().encode(outputText.text)),
      apps:Array.from(document.ApplicationRegistries,app=>({name:app.Name,handle:app.Handle})),seed:document.DrawingVariables.HandleSeed}};
  });
}
