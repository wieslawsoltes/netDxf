# Focused retained-record browser qualification. The complete browser gates
...
report={'scope':'retained-record-io-only','fullSuite':False,'completed':False,'executionMode':'inline-native-esm','digestProvider':'host-sha256','inputTransport':'chunked-json','inputChunkCharacters':262144}
try:
    proof=fingerprints()
    corpus=json.loads((ROOT/('artifacts/retained-record-browser/'+configuration+'/corpus.json')).read_text(encoding='utf-8'))
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if report.get('comparisons') != 2593:
                report['completed']=False
                report['fatal']='Incomplete retained-record browser comparison count.'
    ...
    output=ROOT/'artifacts/retained-record-browser'/configuration/'results.json'
