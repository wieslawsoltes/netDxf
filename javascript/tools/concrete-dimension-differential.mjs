...
try{
  ...
  if(corpus.length!==1141||new Set(corpus.map(p=>p.name)).size!==1141||corpus.reduce((n,p)=>n+p.request.steps.length,0)!==7959)
    throw new Error('Incomplete or duplicate concrete dimension corpus.');
  ...
  if(runtimeFingerprint()!==proof.runtimeFingerprint||verificationFingerprint()!==proof.verificationFingerprint){report.completed=false;report.fatal='Executable source changed during concrete dimension qualification.';}
  const directory=path.join(javascriptRoot,'artifacts/concrete-dimension-differential',configuration);fs.mkdirSync(directory,{recursive:true});
