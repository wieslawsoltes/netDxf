...
try{
  const corpus=objectReferenceCorpus();
  if(corpus.length!==122||new Set(corpus.map(p=>p.name)).size!==122||corpus.reduce((n,p)=>n+p.request.steps.length,0)!==20428)
    throw new Error('Incomplete or duplicate object reference corpus.');
  ...
  if(runtimeFingerprint()!==proof.runtimeFingerprint||verificationFingerprint()!==proof.verificationFingerprint){report.completed=false;report.fatal='Executable source changed during object reference qualification.';}
  const directory=path.join(javascriptRoot,'artifacts/object-reference-differential',configuration);fs.mkdirSync(directory,{recursive:true});
