# Focused primitive geometry browser qualification. The complete browser gates
...
report={'scope':'primitive-io-only','fullSuite':False,'completed':False,'executionMode':'inline-native-esm','digestProvider':'host-sha256','inputTransport':'chunked-json','inputChunkCharacters':262144}
try:
    proof=fingerprints()
    corpus=json.loads((ROOT/('artifacts/primitive-browser/'+configuration+'/corpus.json')).read_text(encoding='utf-8'))
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if report.get('comparisons') != 6416:
                report['completed']=False
                report['fatal']='Incomplete primitive browser comparison count.'
    ...
    output=ROOT/'artifacts/primitive-browser'/configuration/'results.json'
