...
try {
  const corpus = blockCorpus().concat(insertCorpus());
  if (corpus.length !== 369 || new Set(corpus.map(p=>p.name)).size !== corpus.length ||
      corpus.reduce((n,p)=>n+p.request.steps.length,0) !== 5327)
    throw new Error('Incomplete or duplicate block corpus.');
  ...
  for (const probe of corpus) {
    ...
    if (!Array.isArray(expected) || expected.length !== probe.request.steps.length ||
      ...
      throw new Error('Incomplete block result: '+probe.name);
  ...
  if (runtimeFingerprint() !== proof.runtimeFingerprint || verificationFingerprint() !== proof.verificationFingerprint) {
    report.completed=false; report.fatal='Executable source changed during block qualification.';
  }
  const directory=path.join(javascriptRoot,'artifacts/block-differential',configuration);
