...
const report={...proof,configuration,sourceRef:baseline.ref,completed:false,fatal:null,
  stats:{scenarios:0,observedScenarios:0,requestedOperations:0,operations:0,constructorFailures:0,failures:0,oracleFailures:0},categories:{},failures:[]};
const oracle=new ObservableDictionaryOracleSession();
try{
  const corpus=observableDictionaryCorpus();
  if(corpus.length!==427||new Set(corpus.map(p=>p.name)).size!==427||corpus.reduce((n,p)=>n+p.request.steps.length,0)!==15799)
    throw new Error('Incomplete or duplicate observable dictionary corpus.');
  ...
  for(const probe of corpus){
    ...
    if(!Array.isArray(expected)){report.stats.observedScenarios++;report.stats.constructorFailures++;if(!isDeepStrictEqual(expected,actual)){report.stats.failures++;category.failures++;report.failures.push({name:probe.name,expected,actual});}continue;}
  ...
  if(runtimeFingerprint()!==proof.runtimeFingerprint||verificationFingerprint()!==proof.verificationFingerprint){report.completed=false;report.fatal='Executable source changed during observable dictionary qualification.';}
  const directory=path.join(javascriptRoot,'artifacts/observable-dictionary-differential',configuration);fs.mkdirSync(directory,{recursive:true});
