...
export function databaseModelWire(value,wire) {
  if(value instanceof api.DxfGeoMeshPoint)return {type:'DxfGeoMeshPoint',source:wire(value.Source),target:wire(value.Target)};
  if(value instanceof api.DxfGeoMeshFace)return {type:'DxfGeoMeshFace',first:value.First,second:value.Second,third:value.Third};
  ...
  if(value instanceof api.DxfGeoData) {
    const fields={};for(const name of ['Version','CoordinateType','DesignPoint','ReferencePoint','UpDirection','NorthDirection',
      ...
      'ObservationFrom','ObservationTo','ObservationCoverage'])fields[name]=wire(value[name]);
    return {common,fields,host:reference(value.HostBlock),points:Array.from(value.MeshPoints,wire),faces:Array.from(value.MeshFaces,wire)};
  }
  if(value instanceof api.DxfVbaProject)return {common,length:value.DataLength,data:wire(value.Data),chunks:Array.from(value.Chunks,wire)};
