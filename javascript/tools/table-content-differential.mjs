...
try{
  const corpus=tableContentCorpus();
  if(corpus.length!==284||new Set(corpus.map(p=>p.name)).size!==284||corpus.reduce((n,p)=>n+p.request.steps.length,0)!==4850)
  ...
  const folder=path.join(javascriptRoot,'artifacts/table-content',configuration);fs.mkdirSync(folder,{recursive:true});fs.writeFileSync(path.join(folder,'results.json'),JSON.stringify(report,null,2)+'\n');
}
