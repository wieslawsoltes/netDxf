...
try{
  const corpus=layoutViewportCorpus();
  if(corpus.length!==580||new Set(corpus.map(p=>p.name)).size!==580||corpus.reduce((n,p)=>n+p.request.steps.length,0)!==3881)
    throw new Error('Incomplete or duplicate layout/viewport corpus.');
  ...
  if(runtimeFingerprint()!==proof.runtimeFingerprint||verificationFingerprint()!==proof.verificationFingerprint){report.completed=false;report.fatal='Executable source changed during layout/viewport qualification.';}
  const directory=path.join(javascriptRoot,'artifacts/layout-viewport-differential',configuration);fs.mkdirSync(directory,{recursive:true});
