...
try{
  const corpus=storedTableCorpus();
  if(corpus.length!==139||new Set(corpus.map(p=>p.name)).size!==139||corpus.reduce((n,p)=>n+p.request.steps.length,0)!==1679)
  ...
  const folder=path.join(javascriptRoot,'artifacts/stored-table',configuration);fs.mkdirSync(folder,{recursive:true});fs.writeFileSync(path.join(folder,'results.json'),JSON.stringify(report,null,2)+'\n');
}
