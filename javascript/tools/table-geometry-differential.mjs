...
try{
  const corpus=tableGeometryCorpus();
  if(corpus.length!==270||new Set(corpus.map(p=>p.name)).size!==270||corpus.reduce((n,p)=>n+p.request.steps.length,0)!==3966)
  ...
  const folder=path.join(javascriptRoot,'artifacts/table-geometry',configuration);fs.mkdirSync(folder,{recursive:true});fs.writeFileSync(path.join(folder,'results.json'),JSON.stringify(report,null,2)+'\n');
}
