...
try {
  const corpus=transportSectionsCorpus();
  if(corpus.length!==1165||new Set(corpus.map(p=>p.name)).size!==1165||corpus.reduce((n,p)=>n+p.request.steps.length,0)!==3473)throw new Error('Incomplete or duplicate section corpus.');
  ...
  for(const probe of corpus) {
    ...
    const observed=await oracle.observe(probe.request),actual=ValidateTransportObservation(transportSectionsCall(probe.request),probe.request.steps.length);
    ...
    const expected=ValidateTransportObservation(observed.value,probe.request.steps.length);report.stats.observedScenarios++;report.stats.operations+=expected.length;
  ...
  const out=path.join(javascriptRoot,'artifacts/transport-sections',configuration);fs.mkdirSync(out,{recursive:true});fs.writeFileSync(path.join(out,'results.json'),JSON.stringify(report,null,2)+'\n');
}
