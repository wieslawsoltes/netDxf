...
try {
  const corpus=environmentIOCorpus();
  if(corpus.length!==1760||new Set(corpus.map(p=>p.name)).size!==1760||corpus.reduce((sum,p)=>sum+p.request.steps.length,0)!==10173)throw new Error('Incomplete or duplicate environment payload corpus.');
  ...
  for(const probe of corpus) {
    ...
    const observed=await oracle.observe(probe.request),actual=ValidateEnvironmentObservation(environmentIOCall(probe.request),probe.request.steps.length);
    ...
    const expected=ValidateEnvironmentObservation(observed.value,probe.request.steps.length);report.stats.observedScenarios++;report.stats.operations+=expected.length;
  ...
  const out=path.join(javascriptRoot,'artifacts/environment-io',configuration);fs.mkdirSync(out,{recursive:true});fs.writeFileSync(path.join(out,'results.json'),JSON.stringify(report,null,2)+'\n');
}
