...
try {
  ...
  if (corpus.length !== 1497 || new Set(corpus.map(p=>p.name)).size !== corpus.length ||
      corpus.reduce((n,p)=>n+p.request.steps.length,0) !== 3879)
    throw new Error('Incomplete or duplicate drawing-time corpus.');
  ...
  for (const probe of corpus) {
    ...
    if (!Array.isArray(expected) || expected.length !== probe.request.steps.length ||
      ...
      throw new Error('Incomplete drawing-time result: '+probe.name);
  ...
  if (runtimeFingerprint() !== proof.runtimeFingerprint || verificationFingerprint() !== proof.verificationFingerprint) {
    report.completed=false; report.fatal='Executable source changed during drawing-time qualification.';
  }
  const directory=path.join(javascriptRoot,'artifacts/drawing-time-differential',configuration);
