...
try {
  const corpus = geoDataVbaCorpus();
  if (corpus.length !== 398 || new Set(corpus.map(p=>p.name)).size !== corpus.length ||
      corpus.reduce((n,p)=>n+p.request.steps.length,0) !== 2390)
    throw new Error('Incomplete or duplicate geodata/VBA corpus.');
  ...
  for (const probe of corpus) {
    ...
    if (!Array.isArray(expected) || expected.length !== probe.request.steps.length ||
      ...
      throw new Error('Incomplete geodata/VBA result: '+probe.name);
  ...
  if (runtimeFingerprint() !== proof.runtimeFingerprint || verificationFingerprint() !== proof.verificationFingerprint) {
    report.completed=false; report.fatal='Executable source changed during geodata/VBA qualification.';
  }
  const directory=path.join(javascriptRoot,'artifacts/geodata-vba-differential',configuration);
