...
try {
  const corpus = unitFormatCorpus();
  if (corpus.length !== 2938 || new Set(corpus.map(p=>p.name)).size !== corpus.length ||
      corpus.reduce((n,p)=>n+p.request.steps.length,0) !== 45077)
    throw new Error('Incomplete or duplicate unit-format corpus.');
  ...
  for (const probe of corpus) {
    ...
    if (!Array.isArray(expected) || expected.length !== probe.request.steps.length ||
      ...
      throw new Error('Incomplete unit-format result: '+probe.name);
  ...
  if (runtimeFingerprint() !== proof.runtimeFingerprint || verificationFingerprint() !== proof.verificationFingerprint) {
    report.completed=false; report.fatal='Executable source changed during unit-format qualification.';
  }
  const directory=path.join(javascriptRoot,'artifacts/unit-format-differential',configuration);
