...
try {
  const corpus=retainedRecordIOCorpus();
  if(corpus.length!==2593||new Set(corpus.map(p=>p.name)).size!==2593||corpus.reduce((n,p)=>n+p.request.steps.length,0)!==5909)throw new Error('Incomplete or duplicate retained-record corpus.');
  ...
  for(const probe of corpus) {
    ...
    const observed=await oracle.observe(probe.request),actual=ValidateDatabaseObservation(retainedRecordIOCall(probe.request),probe.request.steps.length,'payload');
  ...
  const out=path.join(javascriptRoot,'artifacts/retained-record-io',configuration);fs.mkdirSync(out,{recursive:true});fs.writeFileSync(path.join(out,'results.json'),JSON.stringify(report,null,2)+'\n');
}
