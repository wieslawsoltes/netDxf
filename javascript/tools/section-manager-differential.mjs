...
try{
  const corpus=sectionManagerCorpus();
  if(corpus.length!==174||new Set(corpus.map(p=>p.name)).size!==174||corpus.reduce((sum,p)=>sum+p.request.steps.length,0)!==5406)
  ...
  const folder=path.join(javascriptRoot,'artifacts/section-manager',configuration);fs.mkdirSync(folder,{recursive:true});fs.writeFileSync(path.join(folder,'results.json'),JSON.stringify(report,null,2)+'\n');
}
