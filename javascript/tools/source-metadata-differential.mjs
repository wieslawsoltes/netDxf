...
try {
  const corpus=sourceMetadataCorpus();
  if(corpus.length!==420||new Set(corpus.map(p=>p.name)).size!==420||corpus.reduce((sum,p)=>sum+p.request.steps.length,0)!==12322)throw new Error('Incomplete or duplicate database payload corpus.');
  ...
  for(const probe of corpus) {
    ...
    const observed=await oracle.observe(probe.request),actual=ValidateDatabaseObservation(sourceMetadataCall(probe.request),probe.request.steps.length,'metadata');
    ...
    const expected=ValidateDatabaseObservation(observed.value,probe.request.steps.length,'metadata');report.stats.observedScenarios++;report.stats.operations+=expected.length;
  ...
  const out=path.join(javascriptRoot,'artifacts/source-metadata',configuration);fs.mkdirSync(out,{recursive:true});fs.writeFileSync(path.join(out,'results.json'),JSON.stringify(report,null,2)+'\n');
}
