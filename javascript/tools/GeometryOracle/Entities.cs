...
internal static partial class Program
{
    private static bool EntityWire(object value, out object? result)
    {
        ...
        if (entity is AcisEntity acis) result=new {common,version=acis.ModelerFormatVersion,chunks=acis.EncodedSatChunks.Select(Wire).ToArray(),lines=acis.SatLines.Select(Wire).ToArray(),history=acis is Solid3D solid3d?solid3d.HistoryHandle:null};
