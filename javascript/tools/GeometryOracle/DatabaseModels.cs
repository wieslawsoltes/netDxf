...
internal static partial class Program
{
    ...
    private static bool DatabaseModelWire(object value,out object? result)
    {
        ...
        if(model is DxfDictionary dictionary) result=new {common,hard=dictionary.IsHardOwner,cloning=(int)dictionary.Cloning,entries=dictionary.Entries.Select(Wire).ToArray(),fallback=dictionary is DxfDictionaryWithDefault fallback?ObjectReferenceWire(fallback.Default):null};
