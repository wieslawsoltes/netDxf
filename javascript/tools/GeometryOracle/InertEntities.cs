...
internal static partial class Program
{
    private static bool InertEntityWire(EntityObject entity, object common, out object? result)
    {
        ...
        else if(entity is Section section) result=new {common,state=section.State,flags=section.Flags,name=Wire(section.Name),vertical=Wire(section.VerticalDirection),top=Wire(section.TopHeight),bottom=Wire(section.BottomHeight),transparency=section.IndicatorTransparency,color=Wire(section.StoredIndicatorColor),nativeColor=Wire(section.StoredNativeIndicatorColor),colorName=Wire(section.IndicatorColorName),vertices=section.Vertices.Select(v=>Wire(v)).ToArray(),back=section.BackLineVertices.Select(v=>Wire(v)).ToArray(),settings=section.GeometrySettings is null?null:new {code=section.GeometrySettings.CodeName,handle=section.GeometrySettings.Handle},settingsPresent=section.HasStoredGeometrySettings,erased=section.IsErased};
        else return false;
        return true;
    }
}
