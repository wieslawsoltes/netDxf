...
internal static partial class Program {
  private static bool CurveRecordWire(object value,out object? result) {
    result=null;
    if(value is PolygonMeshRecord pm){
      var mt=(List<netDxf.IO.DxfTag>)typeof(PolygonMeshRecord).GetField("Tags",BindingFlags.Instance|BindingFlags.NonPublic)!.GetValue(pm)!;
      result=new {type="PolygonMeshRecord",code=pm.CodeName,handle=pm.Handle,isEnd=pm.IsSequenceEnd,version=(int)pm.SourceVersion,blockOwner=pm.UsesBlockRecordOwner,layer=Wire(pm.Layer),linetype=Wire(pm.Linetype),tags=mt.Select(t=>new {code=t.Code,value=Wire(t.Value)}).ToArray(),xdata=pm.XData.Values.Select(Wire).ToArray()};return true;
    }
  ...
  private static bool CurveWire(EntityObject value,object common,out object? result) {
    result=null;
    if(value is PolygonMesh pm)result=new {common,u=(int)pm.U,v=(int)pm.V,vertices=Wire(pm.Vertexes),densityU=(int)pm.DensityU,densityV=(int)pm.DensityV,smooth=(int)pm.SmoothType,closedU=pm.IsClosedInU,closedV=pm.IsClosedInV,records=Wire(pm.VertexRecords),endRecord=Wire(pm.EndSequenceRecord)};
    else if(value is Spline s)result=new {common,controls=Wire(s.ControlPoints),fit=Wire(s.FitPoints),weights=Wire(s.Weights),knots=Wire(s.Knots),degree=(int)s.Degree,method=(int)s.CreationMethod,closed=s.IsClosed,periodic=s.IsClosedPeriodic,start=Wire(s.StartTangent),end=Wire(s.EndTangent),parameterization=(int)s.KnotParameterization,knotTolerance=Wire(s.KnotTolerance),controlTolerance=Wire(s.CtrlPointTolerance),fitTolerance=Wire(s.FitTolerance)};
