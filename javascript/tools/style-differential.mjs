...
const fixtures=['acad.lin','acadiso.lin','ltypeshp.shx'].map(name=>{
  const bytes=fs.readFileSync(path.join(sourceRoot,'TestDxfDocument/Support',name));return {name,bytes,sha256:sha256(bytes)};
});
const corpus=styleCorpus(fixtures.slice(0,2).map(f=>({name:f.name,text:f.bytes.toString('utf8')})),fixtures[2].bytes,process.platform==='win32'?'\r\n':'\n');
...
const report={...proof,sourceRef:baseline.ref,configuration,completed:false,fatal:null,fixtures:fixtures.map(({name,sha256})=>({name,sha256})),
  stats:{scenarios:0,operations:0,textComparisons:0,failures:0},categories:{},failures:[]};
const output=path.join(javascriptRoot,'artifacts/style-differential',configuration);fs.mkdirSync(output,{recursive:true});
...
try {
  ...
  for(const probe of corpus) {
    ...
    if(differences.length){report.stats.failures+=differences.length;category.failures+=differences.length;report.failures.push({name:probe.name,category:probe.category,differences});
      if(report.failures.length<=12)console.error('Style mismatch',JSON.stringify(report.failures.at(-1)));}
  }
