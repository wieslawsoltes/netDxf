...
try{
  const corpus=retainedPolylineCorpus();
  if(corpus.length!==181||new Set(corpus.map(p=>p.name)).size!==181||corpus.reduce((sum,p)=>sum+p.request.steps.length,0)!==5419)
  ...
  for(const probe of corpus){
    ...
    if(differences.length){report.stats.failures+=differences.length;report.failures.push({name:probe.name,differences});if(report.failures.length<=12)console.error(probe.name+': '+differences.length+' differences');}
  }
  ...
  const folder=path.join(javascriptRoot,'artifacts/retained-polyline',configuration);fs.mkdirSync(folder,{recursive:true});fs.writeFileSync(path.join(folder,'results.json'),JSON.stringify(report,null,2)+'\n');
}
