import { Spline, Polyline3D, Polyline3DRecord, Ellipse, Circle, Arc, Polyline2D, Polyline2DRecord } from '../index.js';
...
export function curveWire(value,common,wire) {
  if(value instanceof Spline)return {common,controls:wire(value.ControlPoints),fit:wire(value.FitPoints),weights:wire(value.Weights),knots:wire(value.Knots),degree:value.Degree,method:value.CreationMethod,closed:value.IsClosed,periodic:value.IsClosedPeriodic,start:wire(value.StartTangent),end:wire(value.EndTangent),parameterization:value.KnotParameterization,knotTolerance:wire(value.KnotTolerance),controlTolerance:wire(value.CtrlPointTolerance),fitTolerance:wire(value.FitTolerance)};
