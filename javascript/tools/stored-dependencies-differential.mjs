...
try{
  const corpus=storedDependenciesCorpus();
  if(corpus.length!==738||new Set(corpus.map(p=>p.name)).size!==738||corpus.reduce((n,p)=>n+p.request.steps.length,0)!==5566)
  ...
  const folder=path.join(javascriptRoot,'artifacts/stored-dependencies',configuration);fs.mkdirSync(folder,{recursive:true});fs.writeFileSync(path.join(folder,'results.json'),JSON.stringify(report,null,2)+'\n');
}
