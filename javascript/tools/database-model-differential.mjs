...
const corpus=databaseModelCorpus();
...
const output=path.join(javascriptRoot,'artifacts/database-model-differential',configuration);fs.mkdirSync(output,{recursive:true});
...
try {
  ...
  for(const probe of corpus) {
    ...
    if(differences.length){report.stats.failures+=differences.length;category.failures+=differences.length;report.failures.push({name:probe.name,category:probe.category,differences});
      if(report.failures.length<=12)console.error('Database model mismatch',JSON.stringify(report.failures.at(-1)));}
  }
