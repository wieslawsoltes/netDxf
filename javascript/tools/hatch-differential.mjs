...
const fixtures = ['acad.pat', 'acadiso.pat'].map(name => {
  const bytes = fs.readFileSync(path.join(sourceRoot, 'TestDxfDocument', 'Support', name));
  return { name, text: bytes.toString('utf8'), sha256: sha256(bytes) };
});
const nativeManifest = JSON.parse(fs.readFileSync(path.join(javascriptRoot, 'native-port-manifest.json')));
const corpus = hatchCorpus(fixtures, process.platform === 'win32' ? '\r\n' : '\n');
...
const report = { ...proof, sourceRef: baseline.ref, configuration, completed: false, fatal: null,
  fixtures: fixtures.map(({ name, sha256 }) => ({ name, sha256 })),
  stats: { scenarios: 0, operations: 0, textComparisons: 0, failures: 0 }, categories: {}, failures: [] };
const output = path.join(javascriptRoot, 'artifacts', 'hatch-differential', configuration); fs.mkdirSync(output, { recursive: true });
...
try {
  ...
  for (const probe of corpus) {
    ...
    for (let i = 0; i < expected.length; i++) if (probe.request.steps[i].kind === 'pat-save' && expected[i].ok && isDeepStrictEqual(expected[i], actual[i])) report.stats.textComparisons++;
    ...
    if (differences.length) {
      ...
      if (report.failures.length <= 8) console.error('Hatch mismatch', JSON.stringify(report.failures.at(-1)));
    }
  }
