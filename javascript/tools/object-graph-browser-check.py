# Focused object-graph browser qualification. The complete browser gates
...
report={'scope':'object-graph-io-only','fullSuite':False,'completed':False,'executionMode':'inline-native-esm','digestProvider':'host-sha256','inputTransport':'chunked-json','inputChunkCharacters':262144}
try:
    proof=fingerprints()
    corpus=json.loads((ROOT/('artifacts/object-graph-browser/'+configuration+'/corpus.json')).read_text(encoding='utf-8'))
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if report.get('comparisons') != 937:
                report['completed']=False
                report['fatal']='Incomplete object-graph browser comparison count.'
    ...
    output=ROOT/'artifacts/object-graph-browser'/configuration/'results.json'
