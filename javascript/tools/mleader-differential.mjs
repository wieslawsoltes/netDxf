...
try {
  const corpus = mleaderCorpus();
  if (corpus.length !== 1030 || new Set(corpus.map(p=>p.name)).size !== corpus.length ||
      corpus.reduce((n,p)=>n+p.request.steps.length,0) !== 8104)
    throw new Error('Incomplete or duplicate mleader corpus.');
  ...
  for (const probe of corpus) {
    ...
    if (!Array.isArray(expected) || expected.length !== probe.request.steps.length ||
      ...
      throw new Error('Incomplete mleader result: '+probe.name);
  ...
  if (runtimeFingerprint() !== proof.runtimeFingerprint || verificationFingerprint() !== proof.verificationFingerprint) {
    report.completed=false; report.fatal='Executable source changed during mleader qualification.';
  }
  const directory=path.join(javascriptRoot,'artifacts/mleader-differential',configuration);
