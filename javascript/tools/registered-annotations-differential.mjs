...
try{
  const corpus=registeredAnnotationsCorpus();
  if(corpus.length!==112||new Set(corpus.map(p=>p.name)).size!==112||corpus.reduce((sum,p)=>sum+p.request.steps.length,0)!==6633)
  ...
  const folder=path.join(javascriptRoot,'artifacts/registered-annotations',configuration);fs.mkdirSync(folder,{recursive:true});fs.writeFileSync(path.join(folder,'results.json'),JSON.stringify(report,null,2)+'\n');
}
