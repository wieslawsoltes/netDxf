...
try {
  const corpus = stringEnumCorpus();
  if (corpus.length !== 167 || new Set(corpus.map(p=>p.name)).size !== corpus.length ||
      corpus.reduce((n,p)=>n+p.request.steps.length,0) !== 28788)
    throw new Error('Incomplete or duplicate string-enum corpus.');
  ...
  for (const probe of corpus) {
    ...
    if (!Array.isArray(expected) || expected.length !== probe.request.steps.length ||
      ...
      throw new Error('Incomplete string-enum result: '+probe.name);
  ...
  if (runtimeFingerprint() !== proof.runtimeFingerprint || verificationFingerprint() !== proof.verificationFingerprint) {
    report.completed=false; report.fatal='Executable source changed during string-enum qualification.';
  }
  const directory=path.join(javascriptRoot,'artifacts/string-enum-differential',configuration);
