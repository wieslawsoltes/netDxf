...
try {
  const corpus=primitiveIOCorpus();
  if(corpus.length!==6416||new Set(corpus.map(p=>p.name)).size!==6416||corpus.reduce((n,p)=>n+p.request.steps.length,0)!==25569)throw new Error('Incomplete or duplicate entity body corpus.');
  ...
  const out=path.join(javascriptRoot,'artifacts/primitive-io',configuration);fs.mkdirSync(out,{recursive:true});fs.writeFileSync(path.join(out,'results.json'),JSON.stringify(report,null,2)+'\n');
}
