...
export class Polyline2DRecord extends DxfObject {
  ...
  CommonEnd=0;XDataStart;Position=Vector3.Zero;StoredVertex=null;GeometryEnd=0;GeometryIndices=new Map();SourceDocument=null;
  ...
  get Vertex(){return this.StoredVertex;}
  ...
  GeometryTags(){
    const result=new Map();if(this.IsSequenceEnd)return result;const v=this.Vertex;if(v===null)throw new NullReferenceException();const add=(c,val)=>result.set(c,new DxfTag(c,val));
    add(10,v.Position.X);add(20,v.Position.Y);add(30,0);if(v.StartWidthOverride!==null)add(40,v.StartWidthOverride);if(v.EndWidthOverride!==null)add(41,v.EndWidthOverride);
    if(this.GeometryIndices.has(42)||v.Bulge!==0)add(42,v.Bulge);if(v.VertexIdentifier!==null)add(91,v.VertexIdentifier);return result;
  }
  ...
  TopologyTagCount(){
    let count=this.XDataStart+this.GeometryTags().size-this.GeometryIndices.size,extension=false,reactors=false;
  ...
  CopyForClone(vertex=null){
    const copy=new Polyline2DRecord(this.CodeName,new ReferenceList(this.Tags));
    for(const k of ['SourceOwner','CommonEnd','XDataStart','Position','GeometryEnd','IdentityIndex','OwnerIndex','ExtensionHandle','UsesBlockRecordOwner','SourceVersion'])copy[k]=Copy(this[k]);copy.StoredVertex=vertex;
    for(const k of ['GeometryIndices','Coordinates','MetadataGroups','OriginalResourceNames'])for(const [a,b] of this[k])copy[k].set(a,b);
