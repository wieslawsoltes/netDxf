...
import { DecodeDxfText, EncodeDxfText, EncodeDxfDatabaseText } from './DxfStringEncoding.js';
const realCodes = new Set([1010,1020,1030,1011,1021,1031,1012,1022,1032,1013,1023,1033,1040,1041,1042]);
export function ReadXDataRecord(chunk, document, detachedRegistry = false) {
  const name=DecodeDxfText(chunk.ReadString()), found={};
  const app=detachedRegistry?new ApplicationRegistry(name):document.ApplicationRegistries.TryGetValue(name,found)?found.value:document.ApplicationRegistries.Add(new ApplicationRegistry(name));
...
const currentVersion = version => typeof version === 'function' ? version() : version;
export function WriteXData(chunk, version, xdata) {
  for (const app of xdata.AppIds) WriteXDataRecords(chunk, version, app, xdata.get_Item(app).XDataRecord);
}
export function WriteXDataRecords(chunk, version, appReg, records, styleStrings = false) {
  chunk.Write(1001, EncodeDxfText(appReg, currentVersion(version)));
  for (const record of records) {
    const code = record.Code, value = record.Value;
    if (code === 1000 || code === 1003) {
      const text = styleStrings
        ? EncodeDxfDatabaseText(value, currentVersion(version)).replaceAll('\0', '\\U+0000').replaceAll('\r', '\\U+000D').replaceAll('\n', '\\U+000A')
        : EncodeDxfText(value, currentVersion(version));
      chunk.Write(code, text);
    } else if (code === 1004) {
      let index = 0;
      while (value.length - index > 127) {
        const part = new Uint8Array(127); part.set(value.subarray(index, index + 127));
        chunk.Write(code, part); index += 127;
      }
