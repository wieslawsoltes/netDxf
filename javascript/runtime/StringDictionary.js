...
import { ArgumentException, ArgumentNullException, ArgumentOutOfRangeException, KeyNotFoundException, InvalidOperationException, NotSupportedException } from './Errors.js';
/** .NET 8 string dictionary slot order, free-slot reuse, and live read-only views. */
export class StringDictionary {
  #index = new Map(); #slots = []; #free = []; #version = 0; #key; #keys; #values;
  constructor(ignoreCase = true) { this.#key = ignoreCase ? OrdinalIgnoreCaseKey : value => value; }
  #canonical(key) {
    if (key == null) throw new ArgumentNullException('key');
    if (typeof key !== 'string') throw new ArgumentException('A string key is required.', 'key');
    return this.#key(key);
  }
  ...
  GetEnumerator(field = null) {
    const owner = this, version = this.#version; let at = 0, current = field === null ? Object.freeze({ Key: null, Value: null }) : null;
    const check = () => { if (version !== owner.#version) throw new InvalidOperationException('Collection was modified.'); };
    return { get Current() { return current; }, MoveNext() { check(); while (at < owner.#slots.length) { const row = owner.#slots[at++]; if (row !== null) { current = field === null ? row : row[field]; return true; } } at = owner.#slots.length + 1; current = field === null ? Object.freeze({ Key: null, Value: null }) : null; return false; },
      Reset() { check(); at = 0; current = field === null ? Object.freeze({ Key: null, Value: null }) : null; }, Dispose() {},
