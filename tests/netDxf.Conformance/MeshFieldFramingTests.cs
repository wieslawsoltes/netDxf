...
#if DEBUG
        try { DxfDocument.Load(input); throw new Exception("Duplicate/orphan public MESH data was admitted."); }
        catch (InvalidDataException error)
        {
            Check(error.Message.Contains("MESH", StringComparison.Ordinal) && error.Message.Contains(diagnostic, StringComparison.Ordinal) && error.Message.Contains(group.ToString(), StringComparison.Ordinal), "Framing diagnostic lost entity/group context: " + error.Message);
        }
#else
        Check(DxfDocument.Load(input) == null, "Duplicate/orphan public MESH data was admitted.");
#endif
