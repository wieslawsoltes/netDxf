...
{
    private static readonly string[] Draft188_ToleranceMaxLabels = { "8.79", "10.12", "90.12°", "10.12", "5.12", "90.12°", "2.12", "7.98" };
    private static readonly string[] Draft188_ToleranceMinLabels = { "8.41", "9.75", "89.75°", "9.75", "4.75", "89.75°", "1.75", "7.60" };
    private static string Draft188_ToleranceLabelExpected(int kind, int row)
    {
        ...
        if (row == 3) return prefix + "{\\H0.5x;\\S" + Draft188_ToleranceMaxLabels[kind] + "^ " + Draft188_ToleranceMinLabels[kind] + ";}";
        string content = row == 1 ? "±0.12" + symbol : "\\S+0.12" + symbol + "^ -0.25" + symbol + ";";
        return "{\\A1;" + prefix + nominal + "{\\H0.5x;" + content + "}}";
    }
    private static Dimension Draft188_ToleranceLabelDimension(int kind, int row)
    {
    ...
    private static Dimension[] Draft188_ToleranceLabelHosts(DxfDocument doc) => doc.Blocks.SelectMany(b => b.Entities).OfType<Dimension>()
    ...
    private static void Draft188_RegisterToleranceLabelTests()
    {
        for (int kind = 0; kind < 8; kind++) for (int row = 0; row < 6; row++) {
            int k = kind, r = row;
            Run($"draft188-tolerance-label/api/{k}/{r}", () => {
                var dim = Draft188_ToleranceLabelDimension(k, r); var tolerance = dim.Style.Tolerances; var clone = (DimensionStyleTolerances)tolerance.Clone();
                string expected = Draft188_ToleranceLabelExpected(k, r); double measurement = dim.Measurement;
        ...
        for (int index = 0; index < properties.Length; index++) {
            int i = index;
            Run($"draft188-tolerance-label/effective/{i}", () => {
                var dim = Draft188_ToleranceLabelDimension(1, 2); var prior = (DimensionStyleTolerances)dim.Style.Tolerances.Clone();
        ...
        foreach (int alignment in new[] { 0, 1, 2 }) foreach (bool negative in new[] { false, true })
            Run($"draft188-tolerance-label/sign-alignment/{alignment}/{negative}", () => {
                var dim = Draft188_ToleranceLabelDimension(1, 2); dim.Style.Tolerances.VerticalPlacement = (DimensionStyleTolerancesVerticalPlacement)alignment;
        ...
        Run("draft188-tolerance-label/alternate-and-scaling", () => {
            var dim = Draft188_ToleranceLabelDimension(1, 2); dim.Style.DimScaleLinear = 2;
            ...
            Equal("{\\A1;20.0000{\\H0.5x;\\S+0.12^ -0.25;}}[{\\A1;ALT:40.000u{\\H0.5x;\\S+0.250^ -0.500;}}]", AltLabel(dim), "Bounds must not use DIMLFAC; alternate uses its own multiplier and precision");
            File.WriteAllText(Path.Combine(ArtifactDirectory, "draft188-tolerance-label-alternate.txt"), AltLabel(dim));
        ...
        Run("draft188-tolerance-label/limits-independent-rounding", () => {
        ...
        for (int index = 0; index < modes.Length; index++) {
            int i = index;
            Run($"draft188-tolerance-label/linear-format/{i}", () => {
                var prior = CultureInfo.CurrentCulture;
                try {
                    CultureInfo.CurrentCulture = CultureInfo.GetCultureInfo("de-DE");
                    var dim = Draft188_ToleranceLabelDimension(1, 2); dim.Style.DimLengthUnits = modes[i]; dim.Style.Tolerances.Precision = 2;
                    ...
                    File.WriteAllText(Path.Combine(ArtifactDirectory, $"draft188-tolerance-label-linear-{i}.txt"), AltLabel(dim));
        ...
        for (int index = 0; index < angleModes.Length; index++) {
            int i = index;
            Run($"draft188-tolerance-label/angular-format/{i}", () => {
                var dim = Draft188_ToleranceLabelDimension(2, 2);
                if (angleModes[i] == AngleUnitType.SurveyorUnits)
                {
                    // Existing public admission is unchanged: this mode is not an angular format.
                    Throws<ArgumentException>(() => dim.Style.DimAngularUnits = angleModes[i]);
                    Equal(AngleUnitType.DecimalDegrees, dim.Style.DimAngularUnits, "Rejected mode mutated style");
                }
                else dim.Style.DimAngularUnits = angleModes[i];
                ...
                File.WriteAllText(Path.Combine(ArtifactDirectory, $"draft188-tolerance-label-angular-{i}.txt"), AltLabel(dim));
                if (i is 2 or 3) {
                    dim.Style.Tolerances.DisplayMethod = DimensionStyleTolerancesDisplayMethod.Limits; dim.Style.Tolerances.Precision = 2;
        ...
        foreach (string field in new[] { "UpperLimit", "LowerLimit" }) foreach (double bad in new[] { double.NaN, double.PositiveInfinity, double.NegativeInfinity })
            Run($"draft188-tolerance-label/reject/{field}/{ParameterBits(bad)}", () => {
                var dim = Draft188_ToleranceLabelDimension(1, 2); typeof(DimensionStyleTolerances).GetProperty(field)!.SetValue(dim.Style.Tolerances, bad);
        ...
        Run("draft188-tolerance-label/reject-precision", () => {
            var dim = Draft188_ToleranceLabelDimension(1, 2); dim.Style.Tolerances.Precision = 9; Throws<ArgumentOutOfRangeException>(() => AltLabel(dim));
        });
        ...
        for (int kind = 0; kind < 8; kind++) for (int placement = 0; placement < 4; placement++) {
            ...
            Run($"draft188-tolerance-label/wire/{version}/{binary}/{k}/{p}", () => {
                var doc = new DxfDocument(version) { BuildDimensionBlocks = true }; doc.Comments.Clear();
                var hosts = Enumerable.Range(0, 6).Select(row => Draft188_ToleranceLabelDimension(k, row)).ToArray();
                ...
                string stem = $"draft188-tolerance-label-{version}-{binary}-{k}-{p}";
                ...
                foreach (bool output in new[] { false, true }) {
                    hosts = Draft188_ToleranceLabelHosts(loaded); Equal(6, hosts.Length, "Label host inventory");
                    for (int row = 0; row < 6; row++) { Equal(Draft188_ToleranceLabelExpected(k, row), hosts[row].Block.Entities.OfType<MText>().Single().Value, "Stored label"); hosts[row].Update(); Equal(Draft188_ToleranceLabelExpected(k, row), AltLabel(hosts[row]), "Regenerated label"); }
                    ...
                    hosts = Draft188_ToleranceLabelHosts(again); for (int row = 0; row < 6; row++) Equal(Draft188_ToleranceLabelExpected(k, row), hosts[row].Block.Entities.OfType<MText>().Single().Value, "Reloaded label");
