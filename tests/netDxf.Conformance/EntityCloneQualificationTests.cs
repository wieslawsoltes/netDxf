...
{
    private static readonly string[] CloneQualificationKinds = { "solid", "trace", "shape", "polyline", "leader" };

    private static void RegisterEntityCloneQualificationTests()
    {
        foreach (string kind in CloneQualificationKinds)
        ...
        foreach (int depth in Enumerable.Range(0, 3))
        {
            ...
            {
                bool r = registered;
                Run($"clone-qualification/model/{k}/{v}/{d}/{r}", () => CloneQualificationModel(k, v, d, r));
            }
            foreach (DxfVersion version in SupportedVersions)
            foreach (bool binary in new[] { false, true })
            {
                DxfVersion profile = version; bool transport = binary;
                Run($"clone-qualification/wire/{k}/{v}/{d}/{profile}/{transport}",
                    () => CloneQualificationWire(k, v, d, profile, transport));
            }
        }
        foreach (int variant in Enumerable.Range(0, 3))
        {
            int v = variant;
            Run($"clone-qualification/leader-annotation/{v}", () => CloneQualificationAnnotation(v));
        }
        Run("clone-qualification/leader-exact-directions", CloneQualificationDirections);
        foreach (DxfVersion version in SupportedVersions)
        foreach (bool binary in new[] { false, true })
        {
            DxfVersion v = version; bool b = binary;
            Run($"clone-qualification/retained-guards/{v}/{b}", () => CloneQualificationRetained(v, b));
        }
    }

    private static EntityObject CloneQualificationSubject(string kind, int variant)
    {
    ...
    private static EntityObject CloneQualificationWrap(EntityObject leaf, int depth)
    {
    ...
    private static EntityObject CloneQualificationLeaf(EntityObject root)
    {
        while (root is Insert insert) root = insert.Block.Entities.Single();
        return root;
    }

    private static void CloneQualificationEqual(EntityObject original, EntityObject copy, bool serialized = false)
    {
    ...
    private static void CloneQualificationModel(string kind, int variant, int depth, bool registered)
    {
        EntityObject original = CloneQualificationSubject(kind, variant);
        EntityObject root = CloneQualificationWrap(original, depth);
        ...
        var copyRoot = (EntityObject)root.Clone(); EntityObject copy = CloneQualificationLeaf(copyRoot);
        CloneQualificationEqual(original, copy);
    ...
    private static byte[] CloneQualificationSave(EntityObject root, DxfVersion version, bool binary, string file)
    {
    ...
    private static void CloneQualificationWire(string kind, int variant, int depth, DxfVersion version, bool binary)
    {
        EntityObject original = CloneQualificationSubject(kind, variant), root = CloneQualificationWrap(original, depth);
        var copy = (EntityObject)root.Clone(); CloneQualificationEqual(original, CloneQualificationLeaf(copy));
        string stem = $"clone-qualification-{kind}-{variant}-{depth}-{version}-{binary}";
        byte[] before = CloneQualificationSave(root, version, binary, stem + "-source.dxf");
        byte[] after = CloneQualificationSave(copy, version, binary, stem + "-clone.dxf");
        ...
        if (kind != "shape")
        {
            ...
            CloneQualificationEqual(original, CloneQualificationLeaf(loaded.Entities.All.Single()), true);
            Equal(0, loaded.Objects.Validate().Count, "reloaded clone graph");
        }
    }

    private static void CloneQualificationAnnotation(int variant)
    {
        var original = (Leader)CloneQualificationSubject("leader", variant);
        ...
        var copy = (Leader)original.Clone(); CloneQualificationEqual(original, copy);
    ...
    private static void CloneQualificationDirections()
    {
    ...
    private static void CloneQualificationRetained(DxfVersion version, bool binary)
    {
