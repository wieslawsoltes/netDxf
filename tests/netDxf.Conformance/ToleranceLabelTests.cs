...
{
    private static readonly string[] TolNominal = { "8.6603", "10.0000", "90°", "Ø10.0000", "R5.0000", "90°", "2.0000", "7.8540" };
    private static readonly string[] TolMaximum = { "8.910", "10.250", "90.250°", "10.250", "5.250", "90.250°", "2.250", "8.104" };
    private static readonly string[] TolMinimum = { "8.535", "9.875", "89.875°", "9.875", "4.875", "89.875°", "1.875", "7.729" };
    private static readonly string[] TolAltNominal = { "17.32", "20.00", "", "20.00", "10.00", "", "4.00", "15.71" };
    private static readonly string[] TolAltMaximum = { "17.82", "20.50", "", "20.50", "10.50", "", "4.50", "16.21" };
    private static readonly string[] TolAltMinimum = { "17.07", "19.75", "", "19.75", "9.75", "", "3.75", "15.46" };

    private static string TolExpected(int kind, int variant)
    {
        ...
        {
            0 => TolNominal[kind],
            1 => "{\\A1;" + TolNominal[kind] + "{\\H0.5x;±0.125" + angle + "}}",
            2 => "{\\A1;" + TolNominal[kind] + "{\\H0.5x;\\S+0.250" + angle + "^ -0.125" + angle + ";}}",
            _ => prefix + "{\\H0.5x;\\S" + TolMaximum[kind] + "^ " + TolMinimum[kind] + ";}"
        };
        if (variant < 4 || angular) return result;
        string nominal = "A:" + TolAltNominal[kind] + "u";
        return result + "[" + (mode switch
        {
            0 => nominal,
            1 => "{\\A1;" + nominal + "{\\H0.5x;±0.25}}",
            2 => "{\\A1;" + nominal + "{\\H0.5x;\\S+0.50^ -0.25;}}",
            _ => "A:{\\H0.5x;\\S" + TolAltMaximum[kind] + "^ " + TolAltMinimum[kind] + ";}u"
        }) + "]";
    }

    private static Dimension TolLabelDimension(int kind, int variant)
    {
    ...
    private static string TolLabel(Dimension dim) => DimensionBlock.Build(dim).Entities.OfType<MText>().Single().Value;
    ...
    private static void RegisterToleranceLabelTests()
    {
        for (int kind = 0; kind < 8; kind++) for (int variant = 0; variant < 8; variant++)
        {
            ...
            Run($"tolerance-label/api/{k}/{v}", () =>
            {
                var dim = TolLabelDimension(k, v); var sourceTolerance = dim.Style.Tolerances;
                var unchanged = DxSnapshot(dim); double measured = dim.Measurement;
                Equal(TolExpected(k, v), TolLabel(dim), "Tolerance generated text");
                Equal(TolExpected(k, v), TextBlockDirect(dim, "DIRECT").Entities.OfType<MText>().Single().Value, "Typed builder");
                ...
                var clone = (Dimension)dim.Clone(); Equal(TolExpected(k, v), TolLabel(clone), "Detached clone label");
                clone.Style.Tolerances.UpperLimit = 9; clone.StyleOverrides.Clear(); unchanged();
                Equal(TolExpected(k, v), TolLabel(dim), "Clone changed source labels");
                dim.UserText = "FIRST<>SECOND<>";
                Equal("FIRST" + TolExpected(k, v) + "SECOND" + TolExpected(k, v), TolLabel(dim), "Repeated placeholders");
                dim.UserText = "FIXED"; Equal("FIXED", TolLabel(dim), "Literal tolerance bypass");
        ...
        {
            int a = alignment;
            Run($"tolerance-label/alignment/{a}/{scale}", () =>
            {
                var dim = TolLabelDimension(1, 2); dim.Style.Tolerances.VerticalPlacement = (DimensionStyleTolerancesVerticalPlacement)a;
                dim.Style.TextFractionHeightScale = scale;
                Equal("{\\A" + a + ";10.0000{\\H" + scale.ToString("G17", CultureInfo.InvariantCulture) + "x;\\S+0.250^ -0.125;}}", TolLabel(dim), "Scoped tolerance alignment/height");
            });
        }
        foreach (double epsilon in new[] { 1e-12, 0.001 })
            Run("tolerance-label/exact-mode/" + epsilon, () =>
            {
                double old = MathHelper.Epsilon;
                try
                {
                    MathHelper.Epsilon = epsilon;
                    var dim = TolLabelDimension(1, 2); dim.Style.Tolerances.UpperLimit = 1; dim.Style.Tolerances.LowerLimit = 1 + 1e-13;
                    Check(TolLabel(dim).Contains("\\S+1.000^ -1.000;"), "Epsilon collapsed unequal bounds");
                    dim.Style.Tolerances.LowerLimit = 1;
                    Check(TolLabel(dim).Contains("±1.000"), "Equal native tolerance bounds not symmetrical");
                }
                finally { MathHelper.Epsilon = old; }
            });
        foreach (bool limits in new[] { false, true }) foreach (bool negative in new[] { false, true })
        {
            Run($"tolerance-label/scale/{limits}/{negative}", () =>
            {
                ...
                Equal(detached, TolLabel(dim), "Measurement scaling must not scale tolerance or pre-round limits");
        ...
        Run("tolerance-label/signed-zero-and-negative", () =>
        {
            var dim = TolLabelDimension(1, 2); var t = dim.Style.Tolerances;
            t.UpperLimit = -0.25; t.LowerLimit = -0.125;
            Equal("{\\A1;10.0000{\\H0.5x;\\S-0.250^ +0.125;}}", TolLabel(dim), "Signed tolerance bounds");
            t.UpperLimit = -0.0; t.LowerLimit = 0.125;
            Check(TolLabel(dim).Contains("\\S0.000^ -0.125;"), "Negative zero tolerance sign");
            t.LowerLimit = 0.0; Check(TolLabel(dim).Contains("±0.000"), "Zero symmetrical tolerance");
            SameDoubleBits(-0.0, t.UpperLimit, "Rendering changed stored negative zero");
        });
        foreach (bool alternate in new[] { false, true })
            Run("tolerance-label/independent-precision-zero/" + alternate, () =>
            {
                var dim = TolLabelDimension(1, alternate ? 6 : 2);
                ...
                Check(TolLabel(dim).Contains(alternate ? "\\S+.5^ -.25;" : "\\S+.25^ -.125;"), "Tolerance zero suppression override");
        ...
        {
            int m = mode;
            Run("tolerance-label/numeric-format/" + m, () =>
            {
                ...
                try
                {
                    ...
                    var dim = TolLabelDimension(1, 1); dim.Style.DimLengthUnits = formats[m]; dim.Style.Tolerances.Precision = 2;
                    dim.Style.Tolerances.UpperLimit = dim.Style.Tolerances.LowerLimit = 12.5;
                    Check(TolLabel(dim).Contains("±" + units[m] + "}"), "Tolerance unit format and explicit precision");
                    ...
                    Check(TolLabel(dim).Contains("\\S+" + numerator + "^ "), "Stack must escape fractional slash");
        ...
        {
            int m = mode;
            Run("tolerance-label/angular-format/" + m, () =>
            {
                var dim = TolLabelDimension(2, 3); dim.Style.DimAngularUnits = angles[m]; dim.Style.AlternateUnits.Enabled = true;
                Equal("{\\H0.5x;\\S" + angleUpper[m] + "^ " + angleLower[m] + ";}", TolLabel(dim), "Angular limits use selected angular units, not DIMLFAC or alternate units");
            });
        }
        Run("tolerance-label/surveyor-policy", () =>
        {
            var dim = TolLabelDimension(2, 2);
        ...
        foreach (double value in new[] { double.NaN, double.PositiveInfinity, double.NegativeInfinity })
            Run("tolerance-label/nonfinite/" + ParameterBits(value), () =>
            {
                var dim = TolLabelDimension(1, 2); dim.Style.Tolerances.UpperLimit = value;
                Throws<ArgumentOutOfRangeException>(() => TolLabel(dim)); Check(dim.Block == null, "Failed generation attached a block");
                dim.UserText = "FIXED"; Equal("FIXED", TolLabel(dim), "Unused nonfinite tolerance affects literal label");
            });
        Run("tolerance-label/limit-overflow", () =>
        {
            var dim = TolLabelDimension(1, 3); dim.Style.DimScaleLinear = 1e307; dim.Style.Tolerances.UpperLimit = double.MaxValue;
            Throws<ArgumentOutOfRangeException>(() => TolLabel(dim));
        });
        Run("tolerance-label/alternate-overflow", () =>
        {
            var dim = TolLabelDimension(1, 6); dim.StyleOverrides[DimensionStyleOverrideType.TolerancesUpperLimit] = new DimensionStyleOverride(DimensionStyleOverrideType.TolerancesUpperLimit, double.MaxValue);
            Throws<ArgumentOutOfRangeException>(() => TolLabel(dim));
        });
        ...
        for (int kind = 0; kind < 8; kind++) for (int placement = 0; placement < 3; placement++)
        {
            ...
            Run($"tolerance-label/wire/{version}/{binary}/{k}/{p}", () =>
            {
                ...
                var dimensions = Enumerable.Range(0, 8).Select(v => TolLabelDimension(k, v)).ToArray();
                ...
                CheckToleranceLabels(doc, k, handles);
                ...
                var loaded = DxfDocument.Load(source) ?? throw new InvalidOperationException("Tolerance source load"); CheckToleranceLabels(loaded, k, handles);
                foreach (bool output in new[] { false, true })
                {
                    ...
                    CheckToleranceLabels(DxfDocument.Load(stream) ?? throw new InvalidOperationException("Tolerance reload"), k, handles);
                }
    ...
    private static void CheckToleranceLabels(DxfDocument doc, int kind, string[] handles)
    {
        ...
        for (int v = 0; v < dims.Length; v++)
        {
            ...
            {
                var text = dim.Block.Entities.OfType<MText>().Single(); Equal(TolExpected(kind, v), text.Value, "Stored/regenerated tolerance text");
            ...
            var clone = (Dimension)dim.Clone(); Equal(TolExpected(kind, v), TolLabel(clone), "Loaded detached clone");
