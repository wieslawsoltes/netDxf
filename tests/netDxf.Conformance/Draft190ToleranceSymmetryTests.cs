...
{
    private static string Draft190_SymmetryLabel(int variant) => variant == 1
        ? "{\\A1;10.0000{\\H0.5x;\\S+0.250^ -0.125;}}"
        : "{\\A1;10.0000{\\H0.5x;±" + (variant == 3 ? "0.500" : variant == 5 ? "0.000" : "0.250") + "}}";

    private static AlignedDimension Draft190_SymmetryDimension(int variant)
    {
    ...
    private static void Draft190_RegisterToleranceSymmetryTests()
    {
        foreach (DxfVersion version in SupportedVersions) foreach (bool binary in new[] { false, true })
        {
            ...
            {
                int v = variant;
                Run($"draft190-tolerance-label/symmetry/{version}/{binary}/{v}", () =>
                {
                    var doc = new DxfDocument(version) { BuildDimensionBlocks = true }; doc.Comments.Clear();
                    var dim = Draft190_SymmetryDimension(v); doc.Entities.Add(dim); doc.DrawingVariables.DimStyle = dim.Style.Name;
                    Equal(Draft190_SymmetryLabel(v), Draft190_TolLabel(dim), "Symmetrical uses the public upper allowance");
                    ...
                    string stem = $"draft190-tolerance-symmetric-{version}-{binary}-{v}";
                    ...
                    Draft190_CheckSymmetry(loaded, v, dim.Handle);
                    foreach (bool output in new[] { false, true })
                    {
                        ...
                        result.Position = 0;
                        Draft190_CheckSymmetry(DxfDocument.Load(result) ?? throw new InvalidOperationException("Symmetric reload"), v, dim.Handle);
                    }
                    Check(source.CanRead, "Symmetry load closed caller stream");
                });
            }
            Run($"draft190-tolerance-label/foreign-symmetry/{version}/{binary}", () =>
            {
                var doc = new DxfDocument(version) { BuildDimensionBlocks = true }; doc.Comments.Clear();
                doc.Entities.Add(Draft190_SymmetryDimension(4));
                ...
                Equal(Draft190_SymmetryLabel(1), Draft190_TolLabel(dim), "Foreign tolerance label");
            });
        }
    }

    private static void Draft190_CheckSymmetry(DxfDocument doc, int variant, string handle)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            dim.Update(); Equal(Draft190_SymmetryLabel(variant), dim.Block.Entities.OfType<MText>().Single().Value, "Symmetry regenerated label");
        }
        Equal(Draft190_SymmetryLabel(variant), Draft190_TolLabel((Dimension)dim.Clone()), "Symmetry clone");
