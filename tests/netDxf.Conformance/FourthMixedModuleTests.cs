...
internal static partial class Program
{
    ...
    private static readonly string[] FourthNames = { "Missing layer", "missing layer", "Ω中", @"literal \U+0041" };
    ...
    private static void FourthMixedGraph(DxfVersion version, bool binary)
    {
        ...
        var copied = destination.Objects.Clone(graph, destination.Objects.Root, "FOURTH_GRAPH",
            new Dictionary<DxfObject, DxfObject>(ReferenceEqualityComparer.Instance) { [external] = mappedExternal });
        ...
        foreach (string type in new[] { "LAYER_FILTER", "OBJECT_PTR", "SPATIAL_INDEX", "VBA_PROJECT" })
        ...
        File.WriteAllText(Path.Combine(ArtifactDirectory, $"fourth-mixed-{version}-{binary}-map.json"),
            JsonSerializer.Serialize(new { version = version.ToString(), binary, pairs, external = new { source = external.Handle, destination = mappedExternal.Handle }, arbitrary }, new JsonSerializerOptions { WriteIndented = true }));
    }
    ...
    private static void FourthAssertGraph(DxfDocument doc, DxfDictionary graph, DxfDictionaryVariable external, string arbitrary)
    {
        ...
        var data = graph["POINTER"].XData["FOURTH_APP"].XDataRecord;
