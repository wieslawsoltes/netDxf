...
**158 scoped feature rows; 6 typed format families; 9 raw-preservation format families. Full AutoCAD DXF capability is not yet achieved.**
...
| P | Typed path exists but is incomplete or not comprehensively verified. |
...
| Target-version/downgrade diagnostics · typed | X | X | X | P | P | P | P | P | P | Selected guards for MESH, MTEXT backgrounds/frames and VIEW plottability. No centralized schema legality, complete per-loss report or transactional file save. [MVER](mesh-version-export.md), [MTBG](mtext-background.md), [VIEW](named-views.md) |
...
| MESH: subdivision mesh · typed | X | X | X | V | V | V | P | P | P | Typed export rejects pre-2010 versions before writes. Core topology, creases, blend flag and counted-list validation are implemented; subentity override grammar and subdivision evaluation remain incomplete. [MVER](mesh-version-export.md), [MBLEND](mesh-blend-crease.md), [MREAD](mesh-read-validation.md) |
...
| HELIX · typed | X | X | X | M | M | M | M | M | M | Helix model and IO. [B](version-feature-matrix-2026-09-12.md) |
...
## 8. Common fields and dependency graphs
...
## 9. Implemented field-level increments
...
| Raw dependency-closed clone/import/remapping · raw | M | M | M | M | M | M | M | M | M | Handle categories exist, but owners/reactors/reference closure, allocation and graph-safe edits are not implemented. [TAG](raw-tags.md), [REC](raw-records.md) |
...
| Independent interoperability and robustness | Native AutoCAD open/AUDIT/save/reopen when an execution environment is available; corpus expansion, total-resource budgets, fuzzing and actual older-runtime execution. |
