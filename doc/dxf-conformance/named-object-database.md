...
Adding an unowned detached graph establishes its owners and registers its objects. Adding a foreign registered graph fails; use `Objects.Clone` to copy it. `Remove(name)` removes a dictionary link only. It neither destroys objects nor changes handles. Removing the last owning entry makes validation fail until an entry is restored; this prevents saving a silently orphaned owned object. `Objects.EraseOwnedTree` explicitly removes a typed ownership subtree after checking incoming references; see [terminal object erasure](typed-object-erasure.md). The raw OBJECTS transaction API also supplies explicit graph-editing operations.
