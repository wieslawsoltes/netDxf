...
namespace netDxf.Entities
{
    public sealed partial class StoredTable
    {
        ...
        public bool? BackingLiteralValuesAgree { get { return this.CompareBackingLiteralValues(); } }
        ...
        private bool? CompareBackingLiteralValues()
        {
            if (this.BackingContent == null || this.Grid == null) return null;
            ...
            if (starts.Count != this.Grid.Cells.Count) return null;
            bool agree = true;
            for (int i = 0; i < starts.Count; i++)
            {
                if (this.Grid.Cells[i].StoredType != 1) return null;
                if (this.Grid.Cells[i].Tags.Count(t => t.Code == 301 && (string)t.Value == "CELL_VALUE") != 1) return null;
                int? entityType = this.Grid.Cells[i].ValueType;
                if (entityType != 0 && entityType != 1 && entityType != 2 && entityType != 4) return null;
                int end = tags.FindIndex(starts[i] + 1, t => t.Code == 309 && (string)t.Value == "LINKEDTABLEDATACELL_END");
                if (end < 0 || (i + 1 < starts.Count && end >= starts[i + 1])) return null;
                var body = tags.GetRange(starts[i] + 1, end - starts[i] - 1);
                var counts = body.Where(t => t.Code == 95).ToList();
                if (counts.Count != 1 || (int)counts[0].Value < 0 || (int)counts[0].Value > 1) return null;
                ...
                if (marker < 0 || body.Count(t => t.Code == 300 && (string)t.Value == "VALUE") != 1) return null;
                ...
                if (value.ValueType != 0 && value.ValueType != 1 && value.ValueType != 2 && value.ValueType != 4) return null;
