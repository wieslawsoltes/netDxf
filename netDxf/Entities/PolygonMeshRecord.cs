...
{
    /// <summary>A retained VERTEX or SEQEND belonging to a loaded ordinary polygon mesh.</summary>
    /// <remarks>
    /// Geometry remains in PolygonMesh.Vertexes. These records expose identity and common metadata;
    ...
    /// </remarks>
    public sealed class PolygonMeshRecord : DxfObject
    {
        ...
        internal PolygonMeshRecord(string codeName, List<DxfTag> tags) : base(codeName)
        ...
        /// <remarks>The structural Owner is always the containing PolygonMesh. This observed producer form is retained independently.</remarks>
        public bool UsesBlockRecordOwner { get; internal set; }
        /// <summary>Gets the current identity written in ordinary group 330.</summary>
        /// <remarks>For the block-record form this follows an explicit move of the containing polygon mesh to another block.</remarks>
        public DxfObject StoredOwner
        { get { return this.UsesBlockRecordOwner ? ((this.Owner as PolygonMesh)?.Owner as netDxf.Blocks.Block)?.Record : this.Owner; } }
        ...
        internal void Validate(DxfDocument document, PolygonMesh owner, bool registered)
        {
            ...
            if (this.SourceDocument != null && !ReferenceEquals(this.SourceDocument, document) || !ReferenceEquals(this.Owner, owner))
                throw new InvalidOperationException("A retained polygon mesh record must stay with its source polygon mesh and document.");
            ...
            if (registered ? !ReferenceEquals(current, this) : current != null && !ReferenceEquals(current, this))
                throw new InvalidOperationException("A retained polygon mesh record has inconsistent registration.");
            if (registered && (this.StoredOwner == null || !ReferenceEquals(document.GetObjectByHandle(this.StoredOwner.Handle), this.StoredOwner)))
                throw new InvalidOperationException("A retained polygon mesh record has an invalid stored owner.");
            ...
            foreach (DxfObject target in this.References)
                if (!ReferenceEquals(document.GetObjectByHandle(target.Handle), target))
                    throw new InvalidOperationException("A retained polygon mesh record resource is outside its source document.");
        }
        ...
        internal PolygonMeshRecord CopyForClone()
        {
            var result = new PolygonMeshRecord(this.CodeName, new List<DxfTag>(this.Tags))
            {
