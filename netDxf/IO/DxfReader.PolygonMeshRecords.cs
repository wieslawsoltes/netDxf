...
namespace netDxf.IO
{
    internal sealed partial class DxfReader
    {
        ...
        private PolygonMesh ReadStoredPolygonMeshSequence(PolylineTypeFlags flags, Vector3 normal, List<XData> xdata, short m, short n)
        {
            ...
            int expected = (int)m * n;
            var records = new List<PolygonMeshRecord>();
            while (this.chunk.Code == 0 && this.chunk.ReadString() == DxfObjectCode.Vertex)
            {
                if (records.Count >= expected) throw new System.IO.InvalidDataException("POLYGONMESH has more VERTEX records than M x N.");
                records.Add(this.ReadStoredPolygonMeshRecord(false));
            }
            if (records.Count != expected) throw new System.IO.InvalidDataException("POLYGONMESH requires exactly M x N VERTEX records.");
            if (this.chunk.Code != 0 || this.chunk.ReadString() != DxfObjectCode.EndSequence)
                throw new FormatException("A POLYGONMESH vertex sequence requires SEQEND.");
            PolygonMeshRecord end = this.ReadStoredPolygonMeshRecord(true);
            var points = new Vector3[expected]; var slots = new PolygonMeshRecord[expected];
            for (int ordinal = 0; ordinal < expected; ordinal++)
            {
                int slot = ordinal / n + (ordinal % n) * m;
        ...
        private PolygonMeshRecord ReadStoredPolygonMeshRecord(bool end)
        {
            ...
            var record = new PolygonMeshRecord(end ? DxfObjectCode.EndSequence : DxfObjectCode.Vertex, tags)
            ...
            for (; i < tags.Count; i++)
            {
                ...
                if (tag.Code == 100)
                {
                    ...
                    string expected = stage == 0 ? "AcDbEntity" : stage == 1 ? "AcDbVertex" : "AcDbPolygonMeshVertex";
                ...
                if (!end && stage == 3)
                {
                    ...
                    if (tag.Code == 70)
                    {
                        if (flagsSeen || (short)tag.Value != 64) throw new FormatException("The retained VERTEX slice requires ordinary polygon mesh vertex flags 64.");
                        flagsSeen = true;
                    }
            ...
            if (stage != (end ? 1 : 3) || !end && (!flagsSeen || record.Coordinates.Count != 3))
                throw new FormatException("Incomplete ordinary polygon mesh VERTEX/SEQEND record.");
            this.RecordSourceObject(record, source); this.loadedPolygonMeshRecords.Add(record);
            return record;
        }
        private void ResolveStoredPolygonMeshRecords()
        {
            foreach (PolygonMeshRecord record in this.loadedPolygonMeshRecords)
            {
                DxfObject owner = this.GetObjectBySourceHandle(record.SourceOwner);
                if (!ReferenceEquals(owner, record.Owner))
                {
                    var block = (record.Owner as PolygonMesh)?.Owner as netDxf.Blocks.Block;
