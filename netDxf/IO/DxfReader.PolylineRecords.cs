...
namespace netDxf.IO
{
    internal sealed partial class DxfReader
    {
        ...
        private Polyline3DRecord ReadStoredPolylineRecord(bool end)
        {
            ...
            var record = new Polyline3DRecord(end ? DxfObjectCode.EndSequence : DxfObjectCode.Vertex, tags) { SourceDocument = this.doc };
            int i = 0;
        ...
        private void ResolveStoredPolylineRecords()
        {
            ...
            {
                if (!ReferenceEquals(this.GetObjectBySourceHandle(record.SourceOwner), record.Owner))
                    throw new FormatException("A VERTEX/SEQEND owner must be its containing source POLYLINE.");
