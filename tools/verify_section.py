...
def main():
    ...
    check({p.name for p in args.directory.glob('section-*.dxf') if not p.name.startswith(('section-settings-','section-manager-','section-membership-'))}==expected,'Expected all 70 SECTION output fixtures')
