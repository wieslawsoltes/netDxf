...
def validate(wire, source_study, before):
    ...
    hatch_body = public(hatch, "AcDbHatch")
    check(first(hatch_body, 10) == (0, 0, 0) and first(hatch_body, 210) == (0, 0, 1),
          "Mixed HATCH plane or elevation changed")
    ...
    check(first(spline, 95) == 6 and first(spline, 96) == 3,
          "Mixed HATCH spline count declarations changed")
    ...
    seed_start = next(i for i, (code, _) in enumerate(hatch_body) if code == 98)
    seeds = hatch_body[seed_start:]
    check(first(seeds, 98) == 1 and [v for c, v in seeds if c == 10] == [(4, -2)],
          "Mixed HATCH transformed seed or count changed")
    ...
    for year, profile in ((2013, "AC1027"), (2018, "AC1032")):
        ...
        for binary in (False, True):
            ...
            for fault in ("view-target", "geometry-target", "vertex-position", "section-height", "hatch-weight", "study-body",
                          ...
                          "hatch-normal", "hatch-elevation", "hatch-seed", "hatch-knot-count", "hatch-control-count", "hatch-seed-count"):
                ...
                elif fault == "hatch-normal":
                    tags = one(changed, "HATCH"); code, value = 210, (0.0, 1.0, 0.0)
                elif fault == "hatch-elevation":
                    tags = one(changed, "HATCH"); code, value = 10, (0.0, 0.0, 10.0)
                elif fault == "hatch-seed":
                    tags = one(changed, "HATCH"); code, value = 10, (999.0, -2.0)
                elif fault == "hatch-knot-count":
                    tags = one(changed, "HATCH"); code, value = 95, 999
                elif fault == "hatch-control-count":
                    tags = one(changed, "HATCH"); code, value = 96, 999
                elif fault == "hatch-seed-count":
                    tags = one(changed, "HATCH"); code, value = 98, 999
                ...
                at = positions[-1] if fault in ("geometry-target", "hatch-seed") else positions[0]
                tags[at] = (code, value)
                try: validate(changed, original, before)
                except ValueError: controls += 1
                else: raise ValueError("Mixed corruption escaped: " + fault)
            outputs += 1
    check(outputs == 4 and controls == 72, "Mixed required inventory changed")
