...
def main(directory):
    ...
    def name(s):
        ...
        return f'tolerance-label-AutoCad{year}-{binary}-{kind}-{placement}-{output}.dxf'
    ...
    reject(lambda: inventory(expected_files|{'tolerance-label-extra.dxf'}))
    symmetry = list(itertools.product(PROFILES, (False, True), range(6), ('source', 'False', 'True')))
    def symmetry_name(s):
        year, binary, variant, output = s
        return f'tolerance-symmetric-AutoCad{year}-{binary}-{variant}-{output}.dxf'
    ...
    symmetry_inventory({p.name for p in directory.glob('tolerance-symmetric-*.dxf')})
    reject(lambda: symmetry_inventory(symmetry_files-{min(symmetry_files)}))
    reject(lambda: symmetry_inventory(symmetry_files|{'tolerance-symmetric-extra.dxf'}))
    controls = 0
    for s in symmetry:
        year, binary, variant, output = s
