...
def expected(kind, row):
    ...
    return prefix + r'{\A1;' + nominal + r'{\H0.5x;' + content + '}}'
...
def main(directory):
    ...
    def name(s):
        y,b,k,p,o=s;return f'tolerance-label-AutoCad{y}-{b}-{k}-{p}-{o}.dxf'
    names={name(s) for s in specs}
    reports={f'tolerance-label-linear-{i}.txt' for i in range(6)} | {f'tolerance-label-angular-{i}.txt' for i in range(5)} | {'tolerance-label-alternate.txt'}
    def inventory(actual):require(actual==names|reports,'Missing or extra tolerance label fixtures')
    inventory({p.name for p in directory.glob('tolerance-label-*')})
    reject(lambda:inventory(names|reports-{min(reports)}));reject(lambda:inventory(names|reports|{'tolerance-label-extra.dxf'}))
    controls=0
    for spec in specs:
    ...
    for i in range(6):
        label=(directory/f'tolerance-label-linear-{i}.txt').read_text()
        def check(value):
    ...
    for i,symbol in enumerate(('°',None,'g','r','°')):
        label=(directory/f'tolerance-label-angular-{i}.txt').read_text()
    ...
    label=(directory/'tolerance-label-alternate.txt').read_text()
    parse(label,[('+0.12','-0.25','^'),('+0.250','-0.500','^')])
    require('20.0000' in label and '40.000' in label and '[ALT:' in label and 'u]' in label,'Alternate measurement/affixes')
