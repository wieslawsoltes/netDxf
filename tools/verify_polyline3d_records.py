...
def producer_sources():
    folder = ROOT / 'tests/fixtures/polyline3d-records'
...
def native_output(path, year, binary, sources):
    fixture, before = sources[year]
    after, _ = output(path, year, binary)
    mapping = fixture['handle_map']
    expected = children(before)
    check(children(after).keys() == expected.keys(), 'Native child identity inventory')
    for handle, packet in expected.items():
        equal(packet, after[handle], 'Native child packet changed')
        check(native.extractor.owner(after[handle]) == mapping['41A'], 'Native ordinary owner')
    check(native.reactor(after[mapping['41E']], mapping['42F']), 'Native VERTEX reactor lost')
    association = after[mapping['42F']]
    check(association[0].value == 'DIMASSOC', 'Native association identity lost')
    check([t.value for t in native.body(association) if t.code == 331] == [mapping['41A'], mapping['41E']], 'Native repeated geometry path changed')
...
def clone_output(path, year, source_binary, sources):
    ...
    check(len(new) == len(old) == 4 and len(set(new)) == 4, 'Clone owned identity inventory')
...
def corruption_controls(folder, sources):
    path = folder / 'vertex-producer-AutoCad2018-False-False.dxf'
...
def main():
    ...
    producer = producer_sources(); originals = native.source_inputs(); expected = set()
    ...
    for year in VERSIONS:
        for source_binary in (False, True):
            for binary in (False, True):
                verify(f'vertex-native-AutoCad{year}-{source_binary}-{binary}.dxf', native_output, year, binary, originals)
                verify(f'vertex-producer-AutoCad{year}-{source_binary}-{binary}.dxf', producer_output, year, source_binary, binary, producer)
            verify(f'vertex-clone-AutoCad{year}-{source_binary}.dxf', clone_output, year, source_binary, producer)
            verify(f'vertex-move-AutoCad{year}-{source_binary}.dxf', move_output, year, source_binary, producer)
    for binary in (False, True):
        for variant in range(6):
            verify(f'vertex-private-{binary}-{variant}.dxf', private_output, binary, variant, producer)
        verify(f'vertex-edited-{binary}.dxf', edited_output, binary, producer)
        verify(f'vertex-metadata-edited-{binary}.dxf', edited_output, binary, producer, True)
        verify(f'vertex-parent-reactor-move-{binary}.dxf', parent_reactor_move, binary, producer)
    check({p.name for p in args.artifacts.glob('vertex-*.dxf')} == expected, 'Expected exact 90-output inventory')
    corruption_controls(args.artifacts, producer)
    print('VERTEX/SEQEND: 90 outputs; 6 pinned native originals/extractions; 12 unchanged producer fixtures; exact child packets, source identities, both owner forms, owned metadata, clones, moves with parent-reactor rebinding, coordinate edits, private groups and six actual-output corruption controls passed; zero independent audit errors or repairs.')
